...
$$= \mathbb{E}\left[\int_X\int_X f(x)u(x)u(y)g(y)\,dx\,dy\right] = \int_X\int_X f(x)\,\mathbb{E}[u(x)u(y)]\,g(y)\,dy\,dx$$

$$= \int_X f(x)\left(\int_X k(x,y)g(y)\,dy\right)\,dx = \int_X f(x)\,(I_k g)(x)\,dx = \langle f,\ I_k g\rangle_{L^2}.$$
